...
namespace Business.Concrete
{
  public class CarImageManager : ICarImageService
  {
    ...
    public IDataResult<CarImage> GetFirstImageOfCar(int carId)
    {
      var result = _carImageDal.GetAll(ci => ci.CarId == carId).FirstOrDefault();
      if (result != null)
      {
        return new SuccessDataResult<CarImage>(result);
      }
      result = _carImageDal.Get(ci => ci.CarId == 999);
      return new SuccessDataResult<CarImage>(result);
    }
